...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    await loadDistrictTrainingPdfs();
...
async function loadDistrictTrainingPdfs() {
    try {
        ...
        districtTrainingPdfs = records.map((record) => ({
        ...
        console.error("District training PDF records could not be loaded.", error);
...
async function initializeDistrictPdfCarousel() {
    ...
    try {
        ...
        track.innerHTML = districtPdfSlides.map((item) => {
            ...
            const previewPath = encodeURI(item.pdf.preview);
